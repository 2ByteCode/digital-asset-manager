...
class DAM_Posttype_Assets {
	 ...
	 */
	public function dam_manage_asset_table_columns( $columns ) {

		$new_columns = array(
			'cb'                  => $columns['cb'],
			'featured_image'      => __( 'Image', 'digital-asset-manager' ),
			'title'               => $columns['title'],
			'taxonomy-asset-type' => __( 'Types', 'digital-asset-manager' ),
			'taxonomy-asset-tags' => __( 'Tags', 'digital-asset-manager' ),
			'date'                => $columns['date'],
			'dam_record'          => __( 'Record No.', 'digital-asset-manager' ),
			'dam_version'         => __( 'Version', 'digital-asset-manager' ),
			'dam_status'          => __( 'Status', 'digital-asset-manager' ),
			'taxonomy-dam_token'  => $columns['taxonomy-dam_token'],
			'dam_license'         => __( 'License Key', 'digital-asset-manager' ),
			'dam_actions'         => __( 'Actions', 'digital-asset-manager' ),
		);
	 ...
	 * @return void
	 */
	public function dam_manage_asset_table_custom_columns( $column, $post_id ) {
	...
	public function dam_asset_get_status_html( $total_view, $total_download ) {
		...
		$actions_html = '
			<div class="dam-actions">
				<p><strong>Views:</strong> ' . $total_view . '</p>
				<p><strong>Downloads:</strong> ' . $total_download . '</p>
	 ...
	 * @return array
	 */
	public function dam_manage_asset_token_column( $term_links, $taxonomy, $terms ) {

		if ( 'dam_token' === $taxonomy ) {
				$term_links = array();
			foreach ( $terms as $term ) {
				$term_links[] = '
				<li class="tooltip dam-token-li"><a class="dam-action dam-license-key dashicons-before dashicons-media-code" href="#!" title="' . trim( $term->name ) . '" ><span class="tooltiptext" id="myTooltip">Copy to clipboard</span></a></li>';
			}
		}
